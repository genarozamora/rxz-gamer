...
test('model-specific corrections and stock variants', () => {
  ...
  assert.ok(x3.specs.some(s => s.value.includes('5.2/5.4')));
  assert.ok(!x3.images.some(i => /x3-[23]\.jpg|receiver|included/.test(i)));
  assert.ok(!/manual/i.test(x3.specs.find(s => s.label === 'Incluye').value));
  ...
  assert.ok(!/25–50|extractor/.test(JSON.stringify(f75)));
  const gamesir = products.find(p => p.id === 3);
  assert.ok(!/cable|manual/i.test(gamesir.specs.find(s => s.label === 'Incluye').value));
