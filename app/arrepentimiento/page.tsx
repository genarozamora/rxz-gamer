...
export default function ArrepentimientoPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const code = `RXZ-AR-${Date.now().toString(36).toUpperCase()}`;
      ...
      <div className="mx-auto max-w-xl rounded-3xl border border-white/10 bg-[#09131e] p-6 shadow-2xl sm:p-10">
        ...
        {requestCode ? (
          ...
          <form onSubmit={submit} className="mt-8 space-y-4">
            <input name="full_name" required minLength={2} aria-label="Nombre y apellido" placeholder="Nombre y apellido" className="w-full rounded-xl border border-white/10 bg-[#101c29] p-4 outline-none focus:border-emerald-400" />
            <input name="email" required type="email" aria-label="Correo electrónico" placeholder="Correo electrónico de la compra" className="w-full rounded-xl border border-white/10 bg-[#101c29] p-4 outline-none focus:border-emerald-400" />
            <input name="order_number" required aria-label="Número de pedido" placeholder="Número de pedido" className="w-full rounded-xl border border-white/10 bg-[#101c29] p-4 outline-none focus:border-emerald-400" />
